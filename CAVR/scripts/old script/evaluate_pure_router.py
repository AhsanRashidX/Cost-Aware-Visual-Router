"""
Evaluate pure router (no ColPali) on DocVQA dataset
"""
...
print("="*70)
print("PURE ROUTER EVALUATION - DocVQA")
print("="*70)

# Step 1: Load pure router
print("\n[1/5] Loading pure router...")

try:
    from scripts.pure_router import PureRouter
    router = PureRouter()
...
def classify_docvqa_question(question: str) -> str:
    ...
    # Hybrid indicators (need understanding, not just extraction)
    hybrid_patterns = [
        r'why',
        r'how (does|do|is|are|much|many)',
        r'explain',
    ...
    # Visual indicators (need to see location/layout)
    visual_patterns = [
        r'where (is|are)',
        r'located',
        r'position',
...
output_path = Path('./logs/results/pure_router_docvqa.json')
...
for i, r in enumerate(results[:20]):
    ...
    print(f"   {i+1}. {status} Expected: {r['expected']:6} → Predicted: {r.get('predicted', '?'):6}")
